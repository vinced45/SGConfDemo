...
struct StartSessionLiveActivityIntent: AppIntent, LiveActivityIntent {
    ...
    func perform() async throws -> some IntentResult {
        LiveActivityManager.startLiveActivity(for: targetSession?.data ?? .appIntents)
        return .result()
    }
...
// MARK - Optional Entities don't work well with Siri
struct SpeakSessionLiveActivityIntent: AppIntent, LiveActivityIntent {
    static var title: LocalizedStringResource = "Start Session Live Activity "
    static var description = IntentDescription(
        "Starts the session live activity",
        categoryName: "Session"
